  ...
  return (
    ...
    {((coverList) && (coverList.length!==0)) && <div className={cn("slider-container", styles.container, className)}
      ...
      <div className={cn("navigation-wrapper", styles.wrapper)}>
        ...
        <ul ref={sliderRef} className={cn("keen-slider", styles.slider, { [styles.swait]: !loaded })}
          onMouseEnter={() => {
          ...
          {coverList.map((cover, i) => (
            <li key={`cMi_${cover.id}`} className={cn("list", "keen-slider__slide", styles.item, { [styles.wait]: !loaded })}>
              <CoverBtn  className={cn({ [styles.hidden]: !loaded })}
...
/*
    {!loaded && <ul  className={cn("list", styles.load)}>
            {plugArray.map(i=><li key={`psc_${i}`} className={styles.plug}></li>) }
          </ul>
          }
        */